...
export const packageReducer = (state = initialstate1, action) => {
  const { type, payload } = action;

  switch (type) {
    case Actions.PACKAGEDETAILS:
      return { ...state, user11: payload };
    default:
      return state;
  }
};

export const successReducer = (state = initialstate1, action) => {
  const { type, payload } = action;

  switch (type) {
    case Actions.SUCCESS:
      return { ...state, user12: payload };
    default:
      return state;
  }
};

export const transactionReducer = (state = initialstate1, action) => {
  const { type, payload } = action;

  switch (type) {
    case Actions.TRANSACTION:
      return { ...state, user13: payload };
    default:
      return state;
  }
};
