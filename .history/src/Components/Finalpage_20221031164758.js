...
const Finalpage = () => {
  ...
  const { LoginReducer, PayReducer, SuccessReducer } = useSelector(
    (state) => state
  );
  const state2 = LoginReducer.user3.data;
  const state3 = PayReducer.user10.data.notes.merchant_order_id;
  const state4 = PayReducer.user10.data.order_id;
  console.log(state3, state4);
  const dispatch = useDispatch();
  const handleclick = () => {
    dispatch(successDetails({ state2, state3, state4 }));
    // navigate("/");
  };
    ...
    <>
      ...
      <div>
        <div class="card">
          ...
          <p>{SuccessReducer.user12.data}</p>
        </div>
      </div> */}
    </>
  );
