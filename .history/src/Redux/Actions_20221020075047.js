  ...
  return async (dispatch) => {
    try {
      await axios
        ...
        .then((response) => {
          console.log("arjun", response.data);
  ...
  return async (dispatch) => {
    try {
      await axios
        ...
        .then((response) => {
          console.log("charan", response.data);
...
export const successDetails = ({ state2 }) => {
  let config = {
    headers: {
      id: `${state2.Id}`,
      server_key: "3w99V63pW7tJ7vavGXtCKo8cp",
  ...
  const postData5 = {
    orderId: "order_H8JNnFOF4jcXEK",
    razorpay_payment_id: "GwmfGgPuLJmcCw",
  };

  return async (dispatch) => {
    try {
      await axios
        .post(`${baseUrl}/success`, postData5, config)
...
export const transactionDetails = ({ state2 }) => {
  let config = {
    headers: {
      id: `${state2.Id}`,
      server_key: "3w99V63pW7tJ7vavGXtCKo8cp",
  ...
  return async (dispatch) => {
    try {
      await axios
        .post(`${baseUrl}/transactions`, server_key, config)

        .then((response) => {
          console.log("ntr", response.data);
          dispatch({ type: Actions.TRANSACTION, payload: response.data });
        });
    } catch (err) {
      console.error(err);
    }
